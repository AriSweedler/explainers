...
//   latch      the center button: free face = the INVITATION ("Step through
//              3 steps"); stepped face = the COUNTER ("2 of 3 · quarter");
//              press = step in / step off; no dot or radio glyph
//   pill       the radio-row form used for five steps or fewer; the same
...
//              around the whole panel (what the hover tints); a shadow, so the
...
export function mountStepper(fig, notice) {
  ...
  // every move the stepper makes is instant: the reader asked for that step, not a glide
