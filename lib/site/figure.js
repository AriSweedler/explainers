...
import { controlOf, coerce, snaps, stateTargets, nearestState, visibleIds } from '../core/state.js';
...
export function createFigure(el, compiled, env) {
  ...
  // The PANEL: everything under the canvas (controls, stepper, figcaption) is
  // one region that tints on hover and glows on focus; the stepper owns its
  // click (enter) and keydown (← →) listeners. The figcaption moves in so one
  // element carries the tint; aria-labelledby keeps it as the figure's name.
  ...
  el.style.setProperty('--x-ar', aw / ah);
  ...
  const stepper = spec.notice.steps !== 'none' && spec.notice.states.length ? mountStepper(fig, spec.notice, panel) : null;
  ...
  // A reader's change to a control the step names, that takes the figure off
  // the step's pose, steps off (and stops a glide); a control the step never
  // names (a toggle, a grab) leaves it.
  ...
  if (typeof ResizeObserver !== 'undefined') {
    ...
    // ONE VIEWPORT: the stylesheet caps the canvas at 95vh minus the panel, so the panel's height is a variable
  ...
  fig.dispose = () => {
    ...
    for (const node of el.querySelectorAll(':scope > .x-canvas-box, :scope > .x-panel')) node.remove();
