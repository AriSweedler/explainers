...
// The step whose targets lie closest to the current scope, and whether every
// measurable target already matches within eps: where the stepper lands when
// the reader steps in. Each difference is normalized by its control's range
// (a toggle counts 0/1 toward distance but never toward direction); drag
// components and the camera are ignored; ties go to the earlier step. With
// dir = +1 or -1 only the steps that lie ahead of (or behind) the controls count, so the paddles step to the nearest step in
// their direction; past the last step ahead (or the first behind) they land on
// that end.
