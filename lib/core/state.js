...
// measurable target already matches within eps: where the stepper's latch
// lands when the reader steps in. Each difference is normalized by its
// control's range (a toggle counts 0/1); drag components and the camera are
// ignored; ties go to the earlier step.
export function nearestState(compiled, scope, eps = 1e-6) {
  let best = null;
  for (const st of compiled.spec.notice.states) {
    const { targets } = stateTargets(compiled, st.name);
