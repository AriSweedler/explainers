...
| key hints | the ← → `<kbd>` at the panel's far edges, level with the row, while it is **armed** (keyboard focus inside); fine pointers only; decorative; never inside or beside the row, so the widget is not crowded | `kbd.x-key[aria-hidden]`, siblings of the row inside `.x-stepper` |
...
| armed | focus anywhere inside the panel: the glow shows; with focus inside the stepper row, ← → Home End act (unmodified only) and the key hints show; orthogonal to free/stepped | `.x-panel:focus-within`; `.x-stepper:focus-within` for the keys and hints |
