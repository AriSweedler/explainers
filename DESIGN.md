...
| glow | the **armed** indicator, and the only one: a 1 px accent border with a diffuse halo around the whole panel, exactly the area the hover tints, breathing on a 2.2 s cycle (static under reduced motion); never a ring or fill on a child, and always a shadow, so no footprint changes when focus enters or leaves | `.x-panel:has(.x-stepper:focus-within)`, `x-glow` keyframes |
