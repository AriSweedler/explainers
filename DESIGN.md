...
<figure class="x-fig" id="fig-x" data-aspect="3:2" data-mounted>
  ...
  <button class="x-drag-proxy" id="fig-x_drag_p" role="slider" aria-label="p">…</button>   <!-- keyboard nudging; its arrows are its own -->
    ...
    <div class="x-stepper" id="fig-y_steps" data-face="stepped" tabindex="-1">
      ...
      <kbd class="x-key" aria-hidden="true">→</kbd>
      <p class="x-caption" id="fig-y_steps_cap">North.</p>                        <!-- near: the footnote -->
...
| face | `stepped` or `near`; one attribute on the stepper, read by CSS for the counter ink, the segment fill, the caption slot's type and the panel cursor | `.x-stepper[data-face]` |
| stepped | the controls sit exactly on a step, however they got there (a click, a paddle, a key, a drag onto it, a prose link, a deep link, the defaults): the step shows in full ink (solid segment or full-ink counter) with its caption; exact = every target within 1e-6 and at least one target | `data-face="stepped"`, `[aria-current="step"]` on the pill |
| near | the controls sit between steps: the nearest step is marked with an asterisk, the counter muted, the footnote in the caption slot; ‹ › and ← → land on that step or the one beyond it by value | `data-face="near"`, `[aria-current]::after` |
...
| announcer | hidden live text: "Step i of N, name. caption" whenever the reading becomes a step, emptied between steps; one template for both forms | `span.x-sr[aria-live=polite]` |
