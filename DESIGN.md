...
| glow | the **armed** indicator, and the only one: a 1 px accent border with a diffuse outer halo and a soft inner one around the whole row, breathing on a 2.2 s cycle (static under reduced motion); never a ring or fill on a child (that reads as focus on the child), and always a shadow, so the row's footprint never changes when focus enters or leaves | `.x-stepper-row:focus-within` box-shadow + `x-glow` keyframes |
