...
<figure class="x-fig" id="fig-x" data-aspect="3:2" data-mounted>
  ...
  <div class="x-panel">                                            <!-- runtime-made: wraps every .x-ctl, the stepper and the figcaption (moved in at boot; the figure gets aria-labelledby); tints on hover; its background steps in -->
  <div class="x-stepper" id="fig-x_steps">                         <!-- paddle row (more than five steps); five or fewer render as the pill: fieldset.x-stepper-row.x-steps-seg[role=radiogroup][data-face] with legend "Steps", one radio per step, the same caption, key hints and announcer -->
    <div class="x-stepper-row" role="group" aria-label="Steps" data-face="free">   <!-- data-face="stepped" once a step holds -->
      ...
      <button class="x-prev" type="button" aria-label="previous step" aria-disabled="true">‹</button>
      ...
      <button class="x-next" type="button" aria-label="next step" aria-disabled="true">›</button>
      <kbd class="x-key" aria-hidden="true">→</kbd>
    </div>
    ...
    <figcaption id="fig-x_cap">…</figcaption>                       <!-- the author's caption, moved in -->
  </div>                                                           <!-- /.x-panel -->
...
| panel | the region under the canvas that holds the controls, the stepper and the figcaption (the runtime moves the figcaption into it at boot and sets `aria-labelledby` on the figure so the caption stays its accessible name; the source HTML keeps `figure > figcaption`); hovering tints the whole panel, and a click on its background or on a caption steps in, while the controls inside keep their own behaviour | `div.x-panel` (runtime-made; wraps every `.x-ctl`, the `.x-stepper` and the `figcaption`) |
...
| latch | the center button: free face = the **invitation** ("Step through 3 steps") in muted type; stepped face = the **counter** ("2 of 3 · quarter") in full ink; press = step in / step off; its two spans are stacked so the width never changes. The face is told by the words, the paddles' dimming and the caption; there is no dot or radio glyph | `button.x-latch[aria-pressed]` with `span.x-invite` and `span.x-counter` |
| pill | the radio-row form the runtime uses for five steps or fewer; a radio is a step; same caption, hints and announcer; no segment is checked while free | `fieldset.x-steps-seg` |
| glow | the **armed** indicator, and the only one: a 1 px accent border with a diffuse halo around the whole panel, exactly the area the hover tints, whenever anything inside the panel has focus (the stepper, or a slider the reader is dragging), breathing on a 2.2 s cycle (static under reduced motion); never a ring or fill on a child, and always a shadow, so no footprint changes when focus enters or leaves | `.x-panel:focus-within`, `x-glow` keyframes |
| key hints | the ← → `<kbd>` at the panel's far edges, level with the row, while the panel is **armed** and the focused control is not a discrete slider; fine pointers only; decorative; never inside or beside the row | `kbd.x-key[aria-hidden]`, siblings of the row inside `.x-stepper` |
...
| step in / step off | the two transitions: in = a click on the free panel's background or a caption, a paddle, the latch, ← → while armed, a radio, a prose `data-state` link, a deep link, or a **discrete** slider (a values list, or 40 or fewer steps) dragged onto a step's exact values (its stops are steps, so the pill highlights as the knob passes them); off = a **governed** control moved off a step (one the step sets a value for; a toggle the step never names leaves it), Play, Restart, the latch pressed again. Every move the stepper itself makes is instant; prose links and deep links keep their ease | `figure.goto` / `figure.stepOff`, `figure.set(…, 'user')`, `figure.play` |
...
| armed | focus anywhere inside the panel: the glow shows, and ← → (unmodified only) are the stepper's keys, also with focus on a continuous slider (they step, they do not nudge it); a discrete slider keeps its own arrows, since its stops are what they walk, and the key hints hide while it has focus; orthogonal to free/stepped | `.x-panel:focus-within`; the panel routes keydown from `.x-ctl-slider:not(.x-discrete)` to the stepper |
