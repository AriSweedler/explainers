...
| key | type | required | meaning |
|---|---|---|---|
| `steps` | `buttons` \| `segmented` \| `none` | yes | show the stepper (a pill of step buttons up to five states, a paddle row above five; both values mean show) or nothing (states stay addressable) |
...
| `label` | string |  | what the stepper prints for the step; defaults to the name with hyphens as spaces |
...
- posters: every interactive figure has a current first frame as its first child, `<svg class="x-poster" id="<fig>-poster" data-poster="<sha1>">` inline or `<img class="x-poster" src="assets/poster-<fig>.svg" width height data-poster>` when the SVG exceeds 8 KiB (warnings: "no poster", "poster is stale", "poster file ... not found"; `build` fixes all three). The hash covers the emitter version and width, the spec, `data-aspect` and the palette values (the hashed tuple keeps its 3A shape with a trailing, now empty, caption slot, so no 2D poster re-hashed when 3B stopped drawing the caption into scene3d posters). A scene3d figure whose `fallback.poster` is exactly `assets/poster-<fig>.svg` also gets that file written by `build` (the same first frame; warnings "fallback poster ... not found" / "is stale"), and an article with a scene3d figure warns when `dist/explainers-3d.v1.js` is not found beside the runtime include.
- link preview: the head carries `og:title`, `og:type`, `og:url`, `og:description`, `og:site_name` ("Explainers"), `og:image` = `<og:url>assets/og.png` with `og:image:width` 1200, `og:image:height` 630 and `og:image:type` image/png, `twitter:card` summary_large_image, and `<link rel="apple-touch-icon" href="../../assets/apple-touch-icon.png">` (Messages ignores the SVG favicon and shows a 180 x 180 PNG beside the card). `assets/og.png` is the 1200 x 630 card `tools/og-image.mjs` renders from `template/og-card.html` (title, description, the palette `<style>` so `var(--c-*)` resolves, the first `.x-poster`; light scheme only) with a headless-only Chromium; `build` does not produce it, so it is re-rendered by hand when the title, description or hero figure changes. When `og:image` ends in `assets/og.png`, `validate` warns if the file is missing, is not a PNG (the 8-byte signature and the IHDR chunk, no library) or is not 1200 x 630 (`file:line: warning -: og:image assets/og.png not found; run: node tools/og-image.mjs <article>`). `index.html` carries the same tags for `https://explainers.sweedler.com/` and the front page card `assets/og.png` (`node tools/og-image.mjs --site`, which also writes the touch icon).
...
**Added after delivery** (2026-09-24, stepper): every figure boots free; the stepper is a readout of the nearest step plus the one step the last goto asked for (`figure.activeState`); free = muted counter or grey-wash segment + the invitation, near = asterisk + footnote, stepped = full ink or solid + the step's caption; ← → while armed (focus anywhere in the panel) with `<kbd>` hints at the panel's edges; a reader's change to a control the step names that takes the figure off the step, Play or Restart steps off; five steps or fewer render as the pill (buttons with `aria-current`); vocabulary in "Stepper anatomy". (2026-09-24, slider): the knob is an element the runtime draws (`i.x-knob`, last in `.x-track`, so it is in front of the fill), with a grey outline at rest and a halo on hover, focus and drag; discrete sliders get a socket per stop; the vocabulary is fixed in "Slider anatomy". (2026-09-22): slider geometry and ticks. A native range moves the knob so its edge touches the input's ends, so the knob center stops half a knob (20 px) short; the runtime now draws the visible track itself (`div.x-track`, a sibling of the input inset by half the knob on both sides, `--x-pct` measured along it), so the knob center lands exactly on the track ends and the fill is flush with the knob center; the wrapper is one knob wider than the 380/600 px track. A discrete control (`hasStops` in `lib/core/state.js`: a `values` list, or `(max - min) / step <= 40`, or a speed-mode time control's `rates`) gets one `i.x-tick` per stop (2 x 8 px, `--c-muted`, behind the track) where the knob center lands; a continuous control gets none (`lib/controls/slider.js` `stepFractions` / `evenFractions`).
...
**Phase 3B delivered** (2026-09-22): the WebGL figures. `dist/explainers-3d.v1.js` (three.js 0.185.0 + `lib/scene3d/`, built by `tools/build-3d.mjs`), the runtime side in `lib/site/scene3d.js` + `lib/site/figure.js`, the `surface:<objectId>` drag and `geolocate`, projected scene3d posters, and the second Moon figure (`fig-orbit3d`, "The Same Orbit in Three Dimensions").

1. **The chunk.** `node tools/build-3d.mjs` bundles `lib/scene3d/index.js` with esbuild (from `tools/node_modules`; ESM, tree-shaken, minified, es2020, banner naming the generator and the three version) into `dist/explainers-3d.v1.js`: **557 KB raw, 143 KB gzip** (`--check` refuses a stale file; `npm test` runs the same comparison and a 200 KB gzip ceiling). Three is 535 KB of the 557: `WebGLRenderer` (33 KB minified), `WebGLTextures` (30), the physical-lights and shadow-map shader chunks (22 + 10), `Material` (19), `WebGLProgram`/`WebGLPrograms` (17 + 11), `PMREMGenerator` (15), `Object3D` (11), `WebGLState` (11), `BufferGeometry` (10), `WebXRManager` (9): the renderer's fixed cost, which no import shape removes (deep `three/src/...` imports measured 4 KB *larger* gzipped than `from 'three'`, so the adapter imports from `three`). The adapter itself is 22 KB minified. The chunk is not counted by `--budget` (the rule above) and is listed in `dist/integrity.json` for the deploy check and the tests only: it arrives through `import()`, which no `integrity=` attribute can cover; it is same-origin (`script-src 'self'` allows it; the template CSP is unchanged) and its URL is resolved against the runtime's own `src` captured with `document.currentScript` while the bundle evaluates, so it works at `github.io/explainers/`, behind the Worker and from disk. `tools/build-runtime.mjs` allows the one dynamic `import()` in `lib/site/scene3d.js` and still refuses it in every other module (tested).
2. **Runtime side** (`lib/site/scene3d.js`, `lib/site/figure.js`). scene3d figures scaffold at boot like every figure (box, controls, stepper, the build poster over the canvas) and observe at 400 px; the first approach of any scene3d figure runs the WebGL2 check (`canvas.getContext('webgl2')` on a throwaway canvas) and, when it passes, the one `import()` per page (a shared promise); `mountScene3d` then attaches like Scene2D did, draws, and only then sets `data-mounted` / fires `x-fig:mount`, so the poster is the frame until the first WebGL frame. Without WebGL2, or when the import fails, the poster stays, `data-fallback` is set and `fallback.notice` sits over it in a pill (`.x-3d-fallback`); `fallback.poster` is loaded as an `<img>` only when the page has no build poster (an unbuilt copy). A state's `camera` pose eases with the goto transition as three extra numeric targets (`@camera.azimuth|polar|distance`, azimuth taking the short way round via `nearestAzimuth`); a pose asked for before the chunk is up (a deep link at boot) is applied on attach. Prose refs to objects color from the object's `color` token and, on hover, brighten its material and bold its label. `retheme()` re-resolves every material color on a scheme change; `dispose()` tears the renderer, geometries, materials, textures, labels and overlay down.
3. **The adapter** (`lib/scene3d/scene3d.js`, `mountScene3d(canvas, compiled, env)`; env carries `tokens`, `dpr`, `font`, `fmt`, `drags`, `reduced`, `coarse`, `baseUrl`, the canvas box, `requestDraw`, and `env.lib` = the runtime's `createScene2d`, `splitBoxes`, `worldToPx`, `fitCanvas`, `getter`, `compileTemplate`, `renderTemplate`, `compileReadout`, `drawReadout`, `applyModel`, so the chunk holds no second copy of runtime code). `WebGLRenderer({ alpha: true, antialias: true, stencil: <any part has a cut> })` at the runtime's DPR (1|2), transparent over the panel background; the WebGL viewport is the main box minus the split panels and the corner strip (like the 2D drawing), with scissor. Camera: `PerspectiveCamera` at 40° vertical FOV, pose `{ azimuth, polar, distance }` around the target (three's `Spherical` convention: polar from +y, azimuth around y from +z); **arcball** = free turntable rotation by pointer drag (half the width is half a turn) with **momentum** (release velocity measured over the last 100 ms of the drag, capped at 0.2 rad/frame, decaying by `friction` per 60 Hz frame with frame-rate-independent `momentumStep`; off under `prefers-reduced-motion` or `momentum: false`); **orbit** = the same with `min/maxPolar`, `min/maxAzimuth` clamped; **fixed** = no pointer handling; **panorama** = the target follows `lock` and `edgeArrows` draws a small arrow at the view edge toward every visible object off screen, in its color. A touch drag rotates with `touch-action: pan-y` (vertical swipes still scroll the page); a figure with drag controls keeps `touch-action: none`. Light: one `DirectionalLight` along `light.direction` (expressions, re-evaluated each frame) plus `AmbientLight(ambient)`, both scaled by π because three ≥ r155 lights are physical; `shadows: true` turns on a 1024² PCF soft shadow map with cast/receive on globes, spheres, bodies, parts and arrows (discs receive). Objects: **globe** / **sphere** = unit `SphereGeometry(48, 32)` scaled by `radius * scale` (materials `lambert` / `phong` / `standard` / `unlit` = `MeshLambert` / `MeshPhong` / `MeshStandard` / `MeshBasic`; `lut` on a globe = a `ShaderMaterial` that reads the land texture's red channel as height, colors it through the 1-D `lut` ramp and displaces vertices along the normal by height × `exaggeration` × 1 % of the radius; `lut` on anything else falls back to lambert); a globe's `clouds` (1.012 R, transparent, lit) and `outline` (1.003 R, unlit) ride on child spheres; **ring** = a torus whose tube is `width` CSS px at its depth (rebuilt when the radius or the projected width changes by more than 15 %); **disc** = a 96-gon in the object's XZ plane, double-sided, `opacity` as transparency; **arrow** = cylinder shaft + cone head from `from` to `to` (`head` in world units, default 15 % of the length); **body** / **part** = a `.bin` mesh (format below) fetched from `assets`; a part's **cut** = a clipping plane in the object's frame (`plane` × `offset`) with the stencil cap from three's clipping-stencil recipe (back faces increment, front faces decrement, a plane drawn where the stencil is non-zero, in the part's color) and **explode** = a translation of `offset` along `axis`; **label** objects and every object's `label` key = `div.x-3d-label` elements in `div.x-3d-labels` over the canvas, projected each frame (under a sphere's silhouette, at an arrow's tip, at the position otherwise; `offset` replaces the default), text templates re-rendered, colored with `var(--c-<token>)` of the label or its anchor and haloed with `--bg` text shadows, hidden when the anchor is hidden or behind the camera. Common keys `position`, `rotation` (Euler XYZ), `scale`, `color`, `opacity`, `visible` are getters re-evaluated from the scope every draw. Textures: `assets.<name>` is a base path and the files are `<base>@1x.<ext>` / `<base>@2x.<ext>` picked by DPR, the bare path as fallback; color maps are sRGB, the LUT height map linear. Readouts (`shows.readouts`) and split panels draw on a second, transparent 2D canvas (`canvas.x-3d-overlay`) through the runtime's own `drawReadout` and a Scene2D with an empty main panel; in a scene3d a readout's `at` is a fraction of the view box (`[0, 0]` bottom-left, `[1, 1]` top-right). **Surface drags**: the handle is a small sphere (6 px at its depth) at `latLonToPoint(lat, lon)` in the target's local frame, hidden and un-hittable on the far side; a drag raycasts the pointer against the target (a sphere analytically, a body by mesh intersection), sliding to the silhouette point nearest the ray when the pointer leaves the sphere, and yields `[lat, lon]` in **degrees** (`clampLatLon`: lat clamped to ±90, lon wrapped to (−180, 180]); lon 0 is the object's +x axis, 90° E its −z axis, which is where three's `SphereGeometry` puts the center and the right half of an equirectangular texture. `figure.set(name, [lat, lon])` clamps through the same function; the keyboard proxy nudges by 2° (10° with Shift) and reads "37.7° N, 122.4° W"; **geolocate** adds a "Use my location" corner button that calls `navigator.geolocation.getCurrentPosition` (absent when the API is missing, a no-op that re-enables the button on refusal or error).
4. **Posters.** `lib/poster-svg.js` draws a scene3d figure's projected first frame (the CLI section above lists what) through the same `lib/scene3d/camera.js` / `objects.js` / `surface.js` the chunk uses; the caption is no longer drawn into the poster. Measured on the Moon article: `fig-orbit3d` 4.5 KB inline, plus the same bytes in `articles/moon/assets/poster-fig-orbit3d.svg` as its `fallback.poster`.
5. **Tests added**: `test/scene3d.test.mjs` (camera pose math, projection, momentum, Euler transform, lat/lon round trips and clamping, ray-sphere, the `.bin` codec and density URLs, object evaluation against the Moon figure, the scene3d fixture's compile and camera states, the chunk URL, the chunk build: current in `dist/`, ESM, three bundled, under budget, in the manifest), the poster test for the projected scene3d frame (ring runs share their boundary points; a far-side handle is hidden), the CLI test for the fallback poster warnings, and the runtime test that only `lib/site/scene3d.js` contains `import()`. `test/fixtures/pass/scene3d.html` is the minimal valid scene3d figure (orbit camera with limits, sphere, ring, arrow, label, readout, surface drag with geolocate, two camera states). No new error codes, so no new fail fixtures.
6. **Sizes**: runtime 38.8 KB gzip (36.7 before 3B; the loader, the surface clamp and the 3D attach path in `figure.js`), stylesheet 4.5 KB gzip, chunk 143 KB gzip (not counted); Moon critical path 54.6 KB gzip (49.8 after 3A; the second figure, its prose and its 4.4 KB inline poster), Hebrew calendar 90.2 KB (`--budget 170k` passes on both articles with zero warnings).
7. **Verified in headless Chromium** (Playwright's `chrome-headless-shell` with SwiftShader WebGL; screenshots in `preview/3d/`, not committed): the three states in light and dark, the ecliptic toggle, a synthetic pointer drag turning the arcball, a 390 px phone width, the no-WebGL2 fallback (poster + notice) and the no-JavaScript poster; the DOM dump shows `data-mounted`, the three.js canvas, the overlay, the four labels and the readout mirror, and the console is clean.

**Still deferred**: the Playwright suite (`test/browser/poc-1.spec.mjs`: touch drag does not scroll, CLS 0, no 3D fetch before first paint); the DOM behavior of 3B was verified by the headless screenshots above. Not built because nothing in the articles needs them yet, though the code paths exist and are documented: a `globe` with real textures / LUT, `body` / `part` meshes with cuts (no `.bin` asset in the repo), `panorama` edge arrows.
...
| `lib/scene3d/scene3d.js`, `index.js` (lazy chunk `dist/explainers-3d.v1.js`) | `mountScene3d(canvas, compiled, env)` -> the Scene2D-shaped handle (`layout`, `draw`, `hitDrag`, `dragValue`, `readoutText`, `layers`, `readouts`, `highlightObject`, `retheme`, `dispose`) plus `getCamera` / `setCamera` / `cameraTargets` | three.js 0.185.0 adapter bundled by `tools/build-3d.mjs`: camera modes with momentum, lights and shadows, every object kind, materials incl. the LUT shader, cut planes with stencil caps, DOM labels, the overlay canvas for readouts and split panels, surface drag handles, dispose. Three is bundled, so the signature takes `env` (with `env.lib`, the runtime helpers) instead of the sketched `THREE` |
| `lib/scene3d/camera.js`, `surface.js`, `mesh.js`, `objects.js` | pure: `initialPose`, `clampPose`, `eyeFor`, `lookAt`, `project`, `pxPerWorld`, `nearestAzimuth`, `momentumStep`, `rotateEuler`, `toWorld`; `clampLatLon`, `latLonToPoint`, `pointToLatLon`, `rayToSphere`; `encodeMesh`, `parseMesh`, `densityUrl`; `compileObject`, `evalObject`, `circlePoints`, `labelSpecs` | the numbers the chunk, `lib/poster-svg.js` and the tests share; no three.js, no DOM (`surface.js` is also in the runtime bundle for `figure.set`) |
| `lib/site/scene3d.js` | `chunkUrl`, `loadScene3d()` (one `import()` per page), `hasWebGL2`, `mountScene3dFallback` | the runtime's side of a 3D figure: the chunk URL beside the runtime's own `src`, the WebGL2 check, the poster + notice fallback |
...
4. Observe with rootMargin 100px; on first enter, mount. scene3d figures observe at 400px; on the first approach of any of them the runtime checks WebGL2 and runs the one `import()` of `explainers-3d.v1.js` resolved beside its own `src` (captured from `document.currentScript` at load); the poster stays until the first WebGL frame sets `data-mounted`, and stays for good (with the notice) without WebGL2 or when the import fails.
...
figure.goto(stateName, { ease = true })   eases every numeric control/drag/camera over 600 ms (smoothstep), snaps booleans and
                                ...
                                (goto is the only writer of the hash: a pose reached by dragging or Play never writes it)
figure.activeState              the step the last goto asked for, or null; cleared by a reader's change to a control the step
                                names that takes the figure off its pose (a control the step never names leaves it, even during
                                a glide), by play() and restart(); null at boot even when the defaults equal step 1
...
<figure class="x-fig" id="fig-x" data-aspect="3:2" data-mounted>
  ...
  <div class="x-canvas-box">
    ...
    <!-- scene3d only, once the chunk has mounted: the WebGL canvas above is three's (data-engine="three.js r185", touch-action: pan-y), then -->
    <canvas class="x-3d-overlay"></canvas>                    <!-- split panels, readouts, edge arrows; pointer-events: none -->
    <div class="x-3d-labels"><div class="x-3d-label" style="color: var(--c-moon); transform: translate(528px, 306px) translate(-50%, -50%)">Moon</div>…</div>
    <div class="x-3d-fallback"><p class="x-3d-notice">…</p></div>   <!-- only without WebGL2 / on import failure; the figure then carries data-fallback -->
    <button class="x-geolocate" id="fig-x_geo_<name>">Use my location</button>              <!-- bottom-right, surface drags with geolocate -->
  ...
  <div class="x-panel">                                    <!-- runtime-made: holds every .x-ctl, the drag proxies, the stepper and the figcaption (moved in; the figure gets aria-labelledby); tints on hover; glows on :focus-within; cursor: pointer while the stepper is free; a click on anything in it that is not a control or a link enters -->
  ...
  <button class="x-drag-proxy" id="fig-x_drag_p" role="slider" aria-label="p">…</button>   <!-- keyboard nudging; its arrows are its own -->
    ...
    <div class="x-stepper" id="fig-x_steps" data-face="free" data-near tabindex="-1">
      ...
      <div class="x-stepper-row" role="group" aria-label="Steps" aria-describedby="fig-x_steps_cap">
        <button class="x-prev" type="button" aria-label="previous step" aria-disabled="false">‹</button>
        <span class="x-counter">2 of 6 · s1 *</span>
        <button class="x-next" type="button" aria-label="next step" aria-disabled="false">›</button>
      </div>
      <kbd class="x-key" aria-hidden="true">→</kbd>
      <p class="x-caption" id="fig-x_steps_cap">* between steps: stepping lands here or beyond</p>
      <span class="x-sr" aria-live="polite" aria-atomic="true"></span>
    </div>
    <!-- pill form (five steps or fewer), STEPPED at step 2 -->
    <div class="x-stepper" id="fig-y_steps" data-face="stepped" tabindex="-1">
      <kbd class="x-key" aria-hidden="true">←</kbd>
      <div class="x-stepper-row x-steps-seg" role="group" aria-label="Steps" aria-describedby="fig-y_steps_cap">
        <button type="button" value="east">east</button>                          <!-- the text is the step's label (else its name, hyphens as spaces) -->
        <button type="button" value="north" aria-current="step">north</button>   <!-- solid fg on bg, filled to the border; grey wash while free; ::after "*" while near -->
        <button type="button" value="west">west</button>
      </div>
      <kbd class="x-key" aria-hidden="true">→</kbd>
      <p class="x-caption" id="fig-y_steps_cap">North.</p>                        <!-- free: "Step through 3 steps" -->
      <span class="x-sr" aria-live="polite" aria-atomic="true">Step 2 of 3, north. North.</span>
    </div>
    <figcaption id="fig-x_cap">…</figcaption>                <!-- the author's caption, moved in; a click here enters too -->
  </div>                                                   <!-- /.x-panel -->
</figure>
```
...
| stop | an allowed value of a **discrete** slider (`hasStops` in `lib/core/state.js`: a `values` list, or `(max - min) / step <= 40`, or a speed-mode time control's `rates`); a **continuous** slider has none; a control with stops keeps its own ← → (`.x-discrete`) | one `--at` per stop |
...
The words for the stepper under a figure's controls, in code, comments, docs, captions and prose. Nobody says "arrow" for a paddle (arrow is a scene2d layer kind), "thumb", "held" or "release" (drag words), or "tick" (a slider stop's mark). The stepper is a readout with one remembered name: the figure keeps `activeState` (the step the last `goto` asked for, or null) and everything else the stepper shows is derived at every sync from the scope by `nearestState`.

| word | what it is | in the DOM |
|---|---|---|
| stepper | the whole widget under the controls: key hints, row, caption slot, announcer; carries the **face** and the **near** flag; the focus target when the reader enters from the panel background | `.x-stepper#<fig>_steps[data-face=free\|stepped][data-near][tabindex=-1]` |
| step | one of the N named states as the reader meets it ("2 of 3 · quarter"); the counter, the pill and the announcer print its **label** (`label`, else the name with hyphens as spaces); spec, code, deep links and events keep `state` | `notice.states[i]`, `goto(name)`, `#fig-x=name` |
| panel | the region under the canvas holding the controls, the stepper and the figcaption (the runtime moves the figcaption in at boot and sets `aria-labelledby` on the figure); it tints on hover; a click on anything in it that is not a control (`.x-ctl`, the drag proxy) or a link is the stepper's (**enter**); the stepper owns the panel's click and keydown listeners | `div.x-panel` (runtime-made) One viewport per figure: the runtime caps the canvas box at 95vh minus the panel (`--x-panel-h`, measured; `--x-ar` = W/H), so the drawing and its panel are always on screen together; `validate` warns on an aspect taller than wide and on more than three controls under the canvas. |
| row | the one-line strip: paddles and counter, or the pill's segments; one element and one handler for both forms | `.x-stepper-row[role=group][aria-label=Steps][aria-describedby=<fig>_steps_cap]` |
| paddle | a ‹ › button: on a step, the tour's neighbour; between steps, the nearest step that way by value; `aria-disabled` only at an end while stepped, where the click is a no-op; while free both are live doors (at boot ‹ enters step 1, › steps to step 2); always focusable | `button.x-prev`, `button.x-next` |
| counter | plain text "2 of 3 · quarter" (the step's label), plus " *" while near; muted while free, full ink while stepped; not a button (a click on it enters, like the rest of the panel) | `span.x-counter` (paddle form only) |
| pill | the form for five steps or fewer: one button per step (its text the step's label) in the segmented fieldset's clothes, the same height on a phone; the shown step carries `aria-current="step"`: solid (fg on bg) while stepped, a grey wash while free, an absolutely positioned asterisk while near, filling to the pill's border | `.x-stepper-row.x-steps-seg > button[type=button][value=name][aria-current=step]` |
| face | free or stepped; one attribute on the stepper, read by CSS for the counter ink, the segment fill, the caption slot's type and the panel cursor | `.x-stepper[data-face]`; `.x-panel:has([data-face=free]) { cursor: pointer }` |
| free | mode: system-controlled; the figure follows its controls, Play or its defaults; the stepper shows the nearest step in the free look; every figure boots free, even when its defaults equal step 1 | `figure.activeState === null` |
| stepped | mode: human-controlled; the last goto's step is shown and held until a reader's change takes the figure off its pose | `figure.activeState === name` |
| near | free with the controls between steps: the shown step is the nearest, the asterisk marks it, the footnote explains it; exact = every target the step names matches within 1e-6 | `.x-stepper[data-near]`; `nearestState(...).exact === false` |
...
| enter | a click on the panel background, the counter, the caption slot or the figcaption: jump to the shown step (no motion when exact, the nearest when near) and focus the clicked button or the stepper (Safari never focuses a clicked button) | panel click → `jump(shown)`; `(button \|\| .x-stepper).focus({ preventScroll: true })` |
| step in / step off | in = any goto: a paddle, ← → while armed, a segment, enter, a prose link, a deep link; off = a reader's change to a control the step names that takes the figure off the step's pose (it also stops a glide; a control the step never names, such as a toggle or a grab, leaves both alone), Play, Restart | `figure.goto` / `figure.set(…, 'user')` / `figure.play` / `figure.restart` |
| armed | focus anywhere inside the panel: the glow and the key hints show; unmodified ← → are the stepper's keys from every focused thing except a slider with stops, a segmented control or a drag proxy (their arrows already walk stops or nudge; the hints hide while any of the three has focus); orthogonal to the face | `.x-panel:focus-within`; panel keydown, skipping `.x-discrete`, `.x-ctl-segmented` and a `defaultPrevented` key; `:has(… input:focus, .x-drag-proxy:focus) .x-key { opacity: 0 }` |
...
| caption slot | one line under the row that never collapses: stepped → the step's caption (serif); free → the invitation; near → the footnote (both in muted sans); it is the row's `aria-describedby`, so the asterisk has a text equivalent | `p.x-caption#<fig>_steps_cap` |
| invitation | "Step through N steps": the free face's line in the caption slot | `caption.textContent` while free and exact |
| footnote | "* between steps: stepping lands here or beyond": what the asterisk says; it names no key, so it holds for a paddle, ← → and a touch screen alike, and it fits one line at 360 px, so the slot's height never changes | `caption.textContent` while near |
| announcer | hidden live text: "Step i of N, name. caption" when activeState changes to a step, emptied when it clears; one template for both forms; never a nearest flip (Play and drags stay quiet) | `span.x-sr[aria-live=polite][aria-atomic]` |

Ease rule: a request made from the panel is instant, a request made from prose glides, a deep link is instant. Hash rule: goto writes the URL; a pose does not.
...
### Rendering rules (scene3d)

World units are the spec's; y is up; the camera pose is `{ azimuth, polar, distance }` around the origin (or the panorama `lock` object). Every object's `position`, `rotation` (Euler XYZ radians), `scale`, `opacity` and `visible` are re-evaluated from the scope each frame; `color` tokens resolve through `lib/core/tokens.js` and are re-resolved on a scheme change. Rings and discs lie in their object's XZ plane before `rotation`; an arrow runs `from` -> `to` with a `head` in world units; labels are DOM elements projected next to their anchor (below a sphere, at an arrow tip); readouts take `at` as fractions of the view box; split panels draw on the overlay canvas with the 2D rules. The full list is item 3 of "Phase 3B delivered".
...
23. **Surface drags are `[lat, lon]` in degrees**, the one exception to radians-everywhere: the design's own example is `[37.7, -122.4]`, `navigator.geolocation` answers in degrees, and an author converts with `rad()` where trigonometry needs it. lon 0 is the object's +x axis and 90° E its −z axis (three's `SphereGeometry` texture mapping), so a globe's meridians agree with its texture.
24. **Texture files are `<base>@1x.<ext>` / `<base>@2x.<ext>`** beside the path in `assets` (the vocabulary said "textures get @1x/@2x suffixes" without saying where); the bare path is the fallback. Color maps are sRGB; a `lut` globe reads its land map as linear height.
25. **The `.bin` mesh** is `"XMSH"`, version u32 = 1, flags u32 (bit 0 normals, bit 1 indices), vertexCount u32, indexCount u32, then Float32 positions, Float32 normals, Uint32 indices, little-endian; nothing else (`lib/scene3d/mesh.js` encodes and parses it). **The LUT** is any N×1 image whose x axis maps height 0..1 to color; `exaggeration` displaces along the normal by height × exaggeration × 1 % of the radius, so 0 (the default) is a smooth globe.
26. **A scene3d readout's `at` is a fraction of the view box** (`[0, 0]` bottom-left, `[1, 1]` top-right): a 3D figure has no `view`, and readouts are 2D text over the frame. Anchored text next to an object is a `label` object.
27. **`notice.point_at` may name 3D objects** (`SPEC_POINT_AT_MISSING` accepts layer or object ids): a scene3d figure has no layers, and its prose points at objects.
28. **Camera drags use `touch-action: pan-y`** on a scene3d canvas without drag controls: a horizontal finger drag turns the scene, a vertical one still scrolls the page (deviation 4 kept `none` only with drag controls; a 3D figure whose only interaction is the turn gets this middle ground).
29. **Light intensities are scaled by π** so `ambient: 1` means fully lit under three's physical lights (r155+), and `light.direction` is the direction the light comes *from*.
30. **`fallback.poster` doubles as a build output**: a scene3d figure names `assets/poster-<fig>.svg` and `build` writes the projected first frame there (the same bytes as the inline poster); without WebGL2 the runtime keeps the build poster and adds the notice, and uses that `<img>` only on an unbuilt page. `validate` warns when the file is missing or stale, and when the lazy chunk is not beside the runtime include.
31. **The chunk is in `dist/integrity.json` but has no `integrity=` anywhere**: a dynamic `import()` cannot carry one; the entry lets `build-runtime --check` / `build-3d --check` / `npm test` notice a stale chunk and gives the deploy a hash to compare. The template CSP (`script-src 'self'`) already allows the same-origin module.
32. **`mountScene3d(canvas, compiled, env)`, not `(fig, compiled, THREE)`**: three is inside the chunk, and `env.lib` hands the chunk the runtime's Scene2D, readout, layout and expression helpers so nothing is bundled twice. Deep `three/src/...` imports were measured (147.9 KB gzip) against `from 'three'` (143.4 KB) and lost; the renderer is the cost, not the import shape.
33. **The link-preview card is a rendered PNG, not a build output.** `og:image` has to be a raster (Messages, Slack and X do not render SVG), so `tools/og-image.mjs` screenshots `template/og-card.html` with Playwright's `chrome-headless-shell` (`$EXPLAINERS_HEADLESS_SHELL` or the newest under `~/Library/Caches/ms-playwright/`; never Chrome.app, whose every launch steals keyboard focus) into `articles/<slug>/assets/og.png`, and `validate` checks only that the file exists and is a 1200 x 630 PNG, so `build` stays browser-free and idempotent and a stale card is the author's call. The card is light-only (the apps never ask for dark), takes the article's palette and first poster, and the front page card reuses the Moon poster; the `apple-touch-icon` is a PNG from `favicon.svg` for the same reason.
