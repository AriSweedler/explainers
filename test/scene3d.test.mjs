...
test('the 3D chunk is current in dist/, an ESM module with three bundled, under 200 KB gzip, listed in dist/integrity.json', async () => {
  const { code, threeVersion } = await buildChunk();
  assert.equal(threeVersion, '0.185.0');
  assert.equal(fs.readFileSync(path.join(root, OUTFILE), 'utf8'), code, `${OUTFILE} is stale; run: node tools/build-3d.mjs`);
